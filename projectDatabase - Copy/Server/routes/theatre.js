...
router.post('/', async (req, res) => {
    ...
    if (!Capacity || !Rows || !Columns) {
        return res.status(400).json({ 
            error: "Missing required fields: Capacity, Rows, Columns" 
        });
    }

    // Validate data types
    if (typeof Capacity !== 'number' || typeof Rows !== 'number' || typeof Columns !== 'number') {
        return res.status(400).json({ 
            error: "Invalid data types: Capacity, Rows, Columns must be numbers" 
        });
    }
